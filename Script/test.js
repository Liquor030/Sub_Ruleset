...
if (obj.data.data) {
    for (var i = obj.data.data.length - 1; i >= 0; i--) {
        ...
        if (obj.data.data[i].comment_info != null) {
            if (obj.data.data[i].comment_info.video != null) {
                obj.data.data[i].comment_info.video_download.url_list = obj.data.data[i].comment_info.video.url_list;
            }
        }
    }
}
